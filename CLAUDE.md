...
let fullDayByState = { A: null, D: null }; // Issue #130 search stores: full-day, group-filtered, cancelled RETAINED, per direction
let searchOpen = false;           // Issue #130: quick-dial search takeover active
let searchDepFetchToken = 0;      // Issue #130: guards the lazy AState=D search-store fetch (arrivals mode + search open)
let searchDepUnavailable = false; // Issue #130: lazy departures fetch failed → status says unavailable, not loading
```
...
- **Window selector (issue #88, amended by #130 follow-up):** lives in the About-drawer header (the app's settings surface), not the header cluster. Cycles `+2h → +4h → +6h → +8h → +2h`. Forward edge only — the arrivals −40 min backward component never scales with the cycle. `currentForwardHours` is cookie-persisted (`ForwardHours`, owner decision 2026-09-05 superseding #88's in-memory D3) so it survives the `visibilitychange` auto-reload; invalid cookie values fall back to +2. Cycling re-filters the already-fetched day payload (`allSupportedFlights`, the pre-time-filter copy kept in `processFetchedData`) — zero new fetch. `generateFlightNumberButtons()` renders only under an airline pin; with no pin the row is cleared (D2).
- **Midnight crossing (issue #145, revising the #88 truncation):** `windowEnd` is NO longer clamped to `now`'s own UTC+8 calendar day — when `forwardHours` reaches past midnight the window legally extends into tomorrow's early hours, and `getTimeWindow()` reports `crossesMidnight` so callers can react. When the board window crosses, tomorrow's early-hours rows are pulled from the next-day store into the BOARD, chip-marked ("明日"), appended after today's block, filtered by the same window and pins, and never merged into `flightData`/`allSupportedFlights` (the store is memory-only, ingested with an ODate filter and dedupe — #67's marketing-carrier duplicate rows and 2.7% `Gate` population stay contained; `findReturnLeg()`'s same-day pairing still pairs tomorrow departures only against tomorrow arrivals). A today-row whose REVISED time runs past midnight (delay) is today's flight: unchipped, today block. No second-day fetch outside these two gates (search ≥16:00, board window crossing) — #142/#145.
- **A↔D toggle fast path (issue #145):** `toggleFlightMode` passes `allowFreshCacheSkip` — when the other direction's cache is younger than `SWR_TOGGLE_SKIP_MS` (2 min), the board renders from it with zero network requests (no main fetch, no pairing/next-day kicks). Every other trigger (visibilitychange reload, `online` event, pull-to-refresh) still forces the network.
...
## Quick-dial Flight Search (issue #130)

Mobile-first search: 🔍 in the toggle cluster opens a search bar; typing a flight number ("BR178" or "178") renders that flight's gate/carousel **without the 2-hour window** — for today, and from 16:00 UTC+8 also tomorrow (issue #142).

- **Board takeover, no second UI**: results render into `#search-results` (a sibling of `#output` carrying the shared `.flight-board` class) while `#output` and `#planeTypeButtons` are `hidden`; the flight-number and plane-type rows collapse whenever search is OPEN (quick dial replaces them — owner feedback), and the results ignore the window entirely (the window selector lives in the drawer). Seven independent writers of `#output` keep writing the hidden board untouched; closing search only unhides — **never** re-renders (a mode-toggle fetch may still be in flight, and re-rendering then would pair old `flightData` with new headers).
- **Data**: `fullDayByState = { A, D }` holds both directions' full-day, group-filtered stores with cancelled rows RETAINED (searching a cancelled flight answers "cancelled", never "no match"). The current mode's slot is assigned inside `processFetchedData`; departures mode's arrivals slot is fed by `fetchReturnLegArrivals()`; arrivals mode feeds the departures slot via `fetchSearchDepartures()` (lazy `AState=D` fetch, own token, only while search is open — one extra request per refresh, the accepted cost).
- **Next-day store (issue #142 — owner decision 2026-09-13)**: from **16:00 UTC+8** (`shouldFetchNextDay()`, pure + mirrored, unit-tested boundary — +8h just crosses midnight, and the morning flights a night search targets are unreachable in today's payload; the window selector is deliberately IGNORED) a search ALSO lazy-fetches TOMORROW's full-day payload for BOTH directions into `nextDayByState = { date, A, D }` — memory only, never localStorage (the LRU-5 offline cache must not evict today's fallback), refetched every `fetchData()` cycle like the other stores, own token (`nextDayFetchToken`), silent on failure (`nextDayUnavailable` → search stays today-only, next cycle naturally retries). The live API answers future dates (verified 2026-09-13: `ODate=tomorrow` returns 627/632 rows) and the payload carries neighbour-day rows, so ingestion filters `ODate === tomorrow`, keeps the supported groups, and dedupes `(FlightNo, OTime, CityCode)` keeping the first (#67 duplicates). Board invariants hold: next-day rows NEVER enter `flightData`/`allSupportedFlights`.
- **Interleave (issue #142)**: results are `[...matchFlights(todayStore, q, today), ...matchFlights(tomorrowStore, q, tomorrow)]` — today's block first, exact-first sorting inside each store, the 20-row cap applied AFTER the concat (an exact match is never displaced by the other day's rows). The same flight number on both days shows BOTH rows (two separate duties — the crew compares, the app must not guess). Next-day rows carry a `search.tomorrowChip` chip (cancelled rows can carry both chips); tomorrow departures pair their return gate against the tomorrow arrivals pool (`findReturnLeg`'s same-`ODate` rule backstops any mixing). Empty state uses `search.noMatchWithTomorrow` only when the tomorrow store is actually loaded; pending/failed keeps the today-only wording.
- **Matching (digits-only quick dial — owner decision)**: `normalizeFlightQuery()` (NFKC, strip everything that is not a digit — letters like "BR" are ignored entirely) + `matchFlights()` (leading zeros stripped; digit PREFIX match, exact first; other-day rows excluded; cancelled retained). The airline pin does the carrier scoping. Both mirrored in `main.js`.
- **Airline pin applies**: scope is the pin's whole GROUP (`searchScopeGroup()` normalizes `currentACode`, which `displayFlights` overwrites with a tapped flight's own code like `B7`). When the pin hides a match, the empty state offers a one-tap "show all airlines".
- **Presentation**: one table per direction mirroring `displayFlights`' column set, rows otherwise board-plain (owner feedback: no time sub-line, no overflow annotations). Return-gate column sourced from `fullDayByState.A` via parameterized `buildReturnGateCellFrom()`, never `returnLegArrivals` (departures-mode-only). 20-row cap per direction, applied silently (exact matches sort first, so a fully typed number is never hidden). Cancelled rows get a chip + muted styling; unpublished gates get a distinct "TBA" placeholder.
- **State**: `{ open, q }` lives in sessionStorage (`tpe_flight_search`) — per-tab, not a cookie — so the `visibilitychange` reload does not wipe a half-typed query. Restored in `initApp()` between `setupEventListeners()` and `detectLanguage()`: state only, no render; `renderFilteredView`'s hook paints results once data lands.
- **Auto-close on refresh (issue #166)**: a refresh that re-restores a query matching nothing closes the bar itself. One-shot flag `searchAutoCloseArmed` — armed by `restoreSearchSession` and by pull-to-refresh (`fetchData({ forceRefresh: true })` while search is open, which also nulls `fullDayByState.A/D`, `nextDayByState.A/D`, and `searchDepUnavailable` so the settled check waits for the forced cycle's own fetches instead of settling against the previous cycle's rows — PR #167 review finding), disarmed by any typing / `openSearch` / `closeSearch` (digits-only prefix matching makes transient 0-match states normal while typing, so those must never close). Consumed by the first **settled** `renderSearchResults()`: today's two directions resolved (or the lazy departures store declared `searchDepUnavailable`) AND the tomorrow store resolved or not coming (gate closed / failed / stale date) — then, only in the genuinely-empty branch (`!html`, so the pin-hint state never auto-closes: a flight exists behind the pin), it calls the normal `closeSearch()` and returns. A settled render **with** matches just consumes the flag. The one path that never settles: a D-mode arrivals-store failure carries no unavailable flag (`blankResult` leaves `fullDayByState.A` null) — the bar stays open, same as today.
...
- `fetchData()` is stale-while-revalidate (issue #141): a non-forced fetch first paints the cached board when the entry is ≤ `SWR_MAX_AGE_MS` (30 min, `src/utils/swr.js`) old — on any connection — then revalidates online behind a visible "updating" indicator. An entry older than the window is served only when offline (any age, staleness banner), preserving the old offline-fallback contract. The `online` event automatically re-fetches fresh data. The API POST carries a 15s `AbortController` deadline (`API_FETCH_TIMEOUT_MS`) and still passes `cache: 'no-store'` as a belt-and-braces measure against intermediate HTTP caches.
- Revalidation failure never tears down a cache-painted board: the paint stays and the `#swr-status` strip shows "data from N min ago". The generic error is only rendered when nothing could be painted (cold load, no cache). Pull-to-refresh passes `{ forceRefresh: true }`: it skips the cache paint, always issues the real POST, and never blanks the board. The cache key embeds `ODate`, so the 30-min window cannot leak across the UTC+8 midnight rollover. Cache reads/writes are disabled on localhost except under `?e2e-swr=1`.
...
| `ForwardHours` | `2` / `4` / `6` / `8` — the display-window cycle (issue #88, persisted per #130 follow-up) | 400 days (sliding, renewed on change) |
...
- `search.test.js` — quick-dial search matching (issue #130): digits-only normalization (letters ignored), leading zeros, prefix-not-substring, exact-first ordering, cancelled retention, other-day exclusion, and the window-independence property.
- `nextday.test.js` — next-day search gate (issue #142): `shouldFetchNextDay` UTC+8 16:00 boundary (open exactly at 16:00, closed before and after midnight).
...
- `flight-search.spec.js` — quick-dial flight-number search end to end (issue #130): board takeover while a query is active and exact restore on close (single-flight view and pin survive), stacked per-direction tables, airline pin scoping with the show-all escape hatch, cancelled chip, and the sessionStorage restore across a reload. Window-independence itself is unit-tested — on localhost the board is full-day, so E2E cannot distinguish inside/outside the window.
- `nextday-search.spec.js` — cross-day search end to end (issue #142): ODate-aware mock (`setupODateAwareMockRoute`) + frozen page clock; tomorrow rows interleave after today's with the chip, same-number dual rows, marketing-carrier duplicate collapse, cancelled+tomorrow chips, no next-day request before 16:00, and today-only behavior/wording when the tomorrow fetch fails.
- `crossmidnight-board.spec.js` — cross-midnight BOARD end to end (issue #145): +8h window frozen at 22:30 UTC+8 renders tomorrow rows chip-marked after today's block, the apiParams end label carries the actual date, a +2h mid-day window never fetches tomorrow, and the A→D→A toggle within `SWR_TOGGLE_SKIP_MS` issues zero extra requests (fast-path cache render).
