...
- **Board takeover, no second UI**: results render into `#search-results` (a sibling of `#output` carrying the shared `.flight-board` class) while `#output`, `#planeTypeButtons` and `#flightButtons` are `hidden`. Seven independent writers of `#output` keep writing the hidden board untouched; closing search only unhides — **never** re-renders (a mode-toggle fetch may still be in flight, and re-rendering then would pair old `flightData` with new headers).
...
- **Matching**: `normalizeFlightQuery()` (NFKC, uppercase, strip non-alphanumerics) + `matchFlights()` (group-level airline prefix — BR covers B7; unknown prefixes match nothing; leading zeros stripped; digit prefix match, exact first; other-day rows excluded; cancelled retained). Both mirrored in `main.js`.
...
- **Presentation**: one table per direction mirroring `displayFlights`' column set (no sixth Time column — time is a muted second line under the flight code); return-gate column sourced from `fullDayByState.A` via parameterized `buildReturnGateCellFrom()`, never `returnLegArrivals` (departures-mode-only). 20-row cap per direction. Cancelled rows get a chip + muted styling; unpublished gates get a distinct "TBA" placeholder.
...
- `search.test.js` — quick-dial search matching (issue #130): NFKC normalization, group-level airline prefix split, leading zeros, prefix-not-substring, exact-first ordering, cancelled retention, other-day exclusion, and the window-independence property.
